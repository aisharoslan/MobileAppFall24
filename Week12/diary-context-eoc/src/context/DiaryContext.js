...
const postReducer = (state, action) => {
  switch (action.type) {
    ...
    case 'delete_post':
      // filter returns a COPY, not directly modifying state
    ...
    case 'edit_post':
      return state.map((post) => {
        ...
        // } else {
        //   // otherwise, return existing post untouched
        //   return post
        // }
...
export const DiaryProvider = ({children}) => {
  ...
  const [state, dispatch] = useReducer(postReducer, [ {
    ...
    title: 'fake post 1',
    content: 'content'
  }]) // so that don't need to create every time (for testing)

  const addDiaryPost = (title, content, callback) => {
    // axios.post(....) wrap in try catch - all that before we dispatch and do callback
