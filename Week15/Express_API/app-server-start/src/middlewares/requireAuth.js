...
module.exports = (req, res, next) => {
    ...
    // mongoose always lower cases our headers
    ...
    jwt.verify(token, 'MY_SECRET_KEY', async (err, payload) => {
        ...
        // payload contains the information we encoded in our JWT, we need to extract it again
        const { userID } = payload
        // use that userID to look up a user inside our DB
        const user = await User.findById(userID)
        // take that user and assign it to our req object
        req.user = user
        // call the next middleware inside our chain of middlewares (next validation thing) 
        next()
    })
}